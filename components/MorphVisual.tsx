  ...
  return (
    <div className="relative w-full aspect-square max-w-[520px] mx-auto">
      ...
      <AnimatePresence mode="wait">
        <motion.div
          key={`${client.name}-${activePosition}`}
          initial={{ opacity: 0, scale: 0.9, y: 12 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.95, y: -10 }}
          transition={{ duration: 0.45, ease: "easeOut" }}
          className="absolute z-20"
          style={outerPositions[activePosition]}
        >
          <div className="glass-strong rounded-xl px-3.5 py-2.5 border border-cyan-300/30 shadow-xl shadow-cyan-500/15 backdrop-blur-xl">
            <div className="flex items-center gap-2.5">
              <span
                className={`inline-flex h-8 w-10 sm:h-9 sm:w-11 shrink-0 items-center justify-center overflow-hidden rounded-lg border ${
              ...
              >
                <Image
                  ...
                  width={44}
                  height={32}
                  className="max-h-6 sm:max-h-7 w-auto object-contain px-0.5"
                />
              </span>
              <span className="text-[11px] sm:text-xs font-medium text-foreground/95 whitespace-nowrap tracking-wide">
                {client.name}
              </span>
            </div>
          </div>
